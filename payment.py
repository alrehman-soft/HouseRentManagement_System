...
def open_payments():
    ...
    tk.Label(top_frame, text="Remaining Collection", bg="#f8fafc", 
            font=("Segoe UI", 10, "bold italic")).pack(side="left", padx=(0,5))

    remaining_var = tk.StringVar(value="0.00")
    remaining_entry = tk.Entry(top_frame, textvariable=remaining_var, width=15,
                            font=("Segoe UI", 10), state="readonly", disabledbackground="#f0f0f0")
    remaining_entry.pack(side="left", padx=(0,15))
    ...
    pending_count_var = tk.StringVar(value="Total Pending: 0")
    pending_amount_var = tk.StringVar(value="Total Amount: Rs. 0")
    ...
    search_month_combo_bottom.set("February")
    ...
    def update_building_remaining(collector_name, remaining_var):
        # Fetch the remaining collection amount
        conn = get_connection()
        c = conn.cursor()

        # Fetch building name
        c.execute("SELECT building_name FROM rent_collectors WHERE rent_collector_name=?", (collector_name,))
        row = c.fetchone()
        if not row:
        ...
        c.execute("SELECT remaining_amount FROM building_collections WHERE building_name=? AND status='Open'", (building_name,))
        collection = c.fetchone()
        conn.close()

        if collection:
            remaining_var.set(f"{collection[0]:,.2f}")
        else:
            remaining_var.set("0.00")
    ...
    def load_tenant_details(event=None):
        ...
        if tenant_display in tenant_info:
            ...
            # ===== Fetch remaining from building_collections =====
            remaining_amount = fetch_building_remaining(info['building'])
            remaining_var.set(f"{remaining_amount:,.2f}")
        else:
            building_var.set("")
            collector_var.set("Not Assigned")
            rent_var.set("0")
    ...
    def fetch_building_remaining(building_name):
        conn = get_connection()
        c = conn.cursor()
        c.execute("""
    ...
    def load_tenant_rent():
        # Auto-load rent amount
        if not tenant_var.get() or tenant_var.get() not in tenant_combo.tenant_data:
            return
            
        tenant_id = tenant_combo.tenant_data[tenant_var.get()]
        conn = get_connection()
        c = conn.cursor()
        c.execute("SELECT rent_amount FROM tenants WHERE id=?", (tenant_id,))
        result = c.fetchone()
        conn.close()
        
        if result and result[0]:
            rent_var.set(str(result[0]))
        else:
            rent_var.set("0")
    ...
    def get_building_remaining(collector_name):
        conn = get_connection()
        c = conn.cursor()

        # Building of this collector
        c.execute("SELECT building_name FROM rent_collectors WHERE rent_collector_name=?", (collector_name,))
        row = c.fetchone()
        if not row:
            conn.close()
        ...
        # Fetch the open building collection for this building
        c.execute("SELECT remaining_amount FROM building_collections WHERE building_name=? AND status='Open'", (building_name,))
        collection = c.fetchone()
        conn.close()

        if collection:
            return collection[0]  # remaining_amount from collection table
        return 0
    ...
    def save_payment():
        ...
        try:
            ...
            remaining_amount = float(collection[1])  # Direct DB se lo
            ...
            if editing_payment_id is None:
                ...
                remaining_var.set(f"{new_remaining:,.2f}")
            else:
    ...
    def edit_payment():
        ...
        # Get tenant ID from the payment record
        tenant_id = record[1]
    ...
    def load_pending_payments():
        # Load tenants with no payment
        # Clear the table first
        for item in tree_bottom.get_children():
            tree_bottom.delete(item)
        
        selected_month = search_month_var_bottom.get()
        selected_year = search_year_var_bottom.get()

        if not selected_month or not selected_year:
            messagebox.showwarning("Input Required", "Please select month and year")
            return
        
        selected_dt = datetime.strptime(f"{selected_month}-{selected_year}", "%B-%Y")
        if selected_dt > datetime.now():
            messagebox.showwarning("Invalid Selection", "Cannot select a future month/year.")
            return
        ...
        # Get all active tenants who DON'T have a payment
        query = """SELECT 
                    t.id,
                    t.name,
                    t.building_name,
        ...
        c.execute(query, (month_year,))
        pending = c.fetchall()
        conn.close()
        
        total_count = len(pending)
        total_amount = sum(p[5] for p in pending if p[5])
        
        pending_count_var.set(f"Total Pending: {total_count}")
        pending_amount_var.set(f"Total Amount: Rs. {total_amount:,.2f}")
        
        for idx, tenant in enumerate(pending, start=1):
        ...
        tree_bottom.tag_configure('pending', background='#fff3cd')
        
        if total_count == 0:
            tree_bottom.insert("", "end", values=(
                1, "No pending payments", "-", "-", "-", "-", "All Paid"
            ))
    ...
    def on_pending_double_click(event):
        ...
        if len(values) < 2 or values[1] == "No pending payments":
            return
        ...
        for item in tenant_combo['values']:
            if tenant_name in item:
                tenant_var.set(item)
                load_tenant_details()
                # Auto-set month and year from bottom table search
                month_var.set(search_month_var_bottom.get())
                year_var.set(search_year_var_bottom.get())
                messagebox.showinfo("Info", f"Selected {tenant_name}. Please enter payment details.")
                break
...
#     tk.Button(window,text="Building Collection",command=open_building_collection,
#             bg="#31726C",fg="white",font=("Segoe UI", 12, "bold italic"),width=20,height=2, cursor="hand2").pack(padx=5)
...
#             building_var.set(info['building'])
#             rent_var.set(str(info['rent_amount']))
#             load_rent_collector(info['building'])  # collector dictionary ya query se
#         else:
#             building_var.set("")
#             collector_var.set("Not Assigned")
#             rent_var.set("0")
...
#                                     "No active collection found for this building.")
#                 conn.close()
#                 return

#             collection_id = collection[0]
#             remaining_amount = collection[1]

#             if paid > remaining_amount:
#                 messagebox.showerror("Error",
#                                     "Payment exceeds remaining collection amount.")
#                 conn.close()
#                 return
